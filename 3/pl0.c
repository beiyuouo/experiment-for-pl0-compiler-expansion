...
void init() {
    ...
    strcpy(&(word[3][0]), "do");
    strcpy(&(word[4][0]), "else"); // add
    strcpy(&(word[5][0]), "end");
    strcpy(&(word[6][0]), "for"); // add
    strcpy(&(word[7][0]), "if");
    strcpy(&(word[8][0]), "odd");
    strcpy(&(word[9][0]), "procedure");
    strcpy(&(word[10][0]), "read");
    strcpy(&(word[11][0]), "then");
    strcpy(&(word[12][0]), "to"); // add
								...
								if (sym == forsym) {
								    ...
								    if (i == 0) {
								        ...
								        if(table[i].kind != variable) {
								            ...
								            cx1 = cx; // 记录条件比较起始地址
								            gendo(lod, lev-table[i].level, table[i].adr); // 循环变量提到栈顶 
								            memcpy(nxtlev, fsys, sizeof(bool)*symnum);
								            nxtlev[beginsym]=true;
								            expressiondo(nxtlev, ptx, lev); // 处理循环边界表达式 
								            gendo(opr, 0, 13); // 生成条件比较指令 
								            cx2 = cx; // 记录条件跳转地址 
								            gendo(jpc, 0, 0);
								            if (sym == dosym) {
								            	getsymdo; // 直接进入begin语句处理，不需要获取符号 
							                	statementdo(fsys, ptx, lev); // 处理循环体 
								            } else {
								            	error(18);
											}
							                
							                gendo(lod, lev-table[i].level, table[i].adr); // 变量提到栈顶 
							                gendo(lit, 0, 1); // 常数1添加到栈顶
							                gendo(opr, 0, 2); // 循环变量增加1
							                gendo(sto, lev-table[i].level, table[i].adr); // 赋值
							                gendo(jmp, 0, cx1); // 回条件判断
							                code[cx2].a = cx; // 跳出循环
								        }
								    }
                                } else {
